...
## WireFactory workshop background — 2026-09-14

User explicitly requested the same landing-page workshop from another camera angle in WireFactory and publication. Built-in image_gen used the existing luthier-workshop-hero-v1.webp as the room reference. The new static 1672x941 WebP is 157586 bytes; no source PNG is published. Only the WireFactory snapshot and this record change in this publication. Landing and GTRFactory retain their previous bytes.

The CSS backdrop ignores pointer events and has no animation. Dark translucent surfaces preserve circuit/readability. No editor data, solver, geometry or export code changed. Local Wire build PASS3.79s, targeted desktop/mobile move/check/custom E2E PASS4/4 (16.5s). Shared release contracts PASS9/9; Wire refresh and combined build PASS9.40s. Local desktop/mobile background checks PASS: one image request each, no application/HTTP errors, no global horizontal overflow; SVG bytes match the prior release and PDF parses as two pages. Both screenshots visually checked. New Wire sourceSHA: 46f992a74e9bdec3639cb6bd920e3727e0db61dbb8cc3f22f4fae3ee50a3a49f. Evidence: WireFactory output/workshop-background/local/results.json. Public deployment evidence follows after Pages completion.

Published background: [Pages run 34838501898](https://github.com/IlkkaE/fretfactory/actions/runs/34838501898) SUCCESS for commit07a0f7a0c50d62bdfdac1a865e85fd5fe9a5f28c (build22s/deploy10s). Fresh public HTTPS verification passed on desktop/mobile: manifest and all6 artifact hashes match, one successful background request per viewport, no application/HTTP errors or horizontal overflow; SVG remains byte-identical to the prior release, PDF parses as two pages. Evidence: WireFactory output/workshop-background/live/results.json. No physical-device claim.

## SEO and public-source release preparation — 2026-09-15

GTRFactory source commit `1ea5964` and WireFactory source commit `b030f6e` are now public repositories. Their refreshed browser snapshots were installed with `sourceDirty: false` and source SHA-256 values `0f4901030162b2aa8182cbd93367f166d04ba909b173aa588c6d5b5de5b525c8` and `c0d78cbeadbfad130a0885ca8540389a4222b664b8f2b49532e1b67707ba8f2d` respectively. The combined local build includes route-specific root/fretboard metadata, snapshot-owned GTR/Wire metadata, `robots.txt`, and a four-route sitemap. Local automated evidence: release contracts 11/11, Fret tests 63/63, combined build PASS. Post-push evidence: Pages workflow `34955697280` passed for `1df48fb`; GitHub Pages reports an approved `www.fretfactory.fi` certificate and `https_enforced: true`. HTTP root returns `301` to HTTPS, all four canonical routes return `200` with expected title and canonical URL, and `robots.txt` plus `sitemap.xml` are public. Browser-console QA remains unverified because the automation helper exited before window selection; direct HTTP checks do not prove console cleanliness or interactive UI behaviour.
